...
class Station2ConceptualAnalysis extends base_station_1.BaseStation {
    ...
    async execute(input, options) {
        ...
        const context = this.buildContextFromStation1(station2Input.station1Output, station2Input.text);
        try {
            const [storyStatements, threeDMap, hybridGenreOptions, themeAnalysis, targetAudience,] = await Promise.all([
            ...
            const [elevatorPitch, genreMatrix, dynamicTone, artisticReferences, marketAnalysis,] = await Promise.all([
            ...
            return {
                ...
                metadata: {
                    ...
                    status: "Success",
                    processingTime,
                    confidenceScore: this.calculateConfidenceScore(storyStatements, threeDMap, themeAnalysis),
                },
            };
        }
        catch (error) {
    ...
    validate3DMap(parsed) {
        return {
            ...
            temporalDevelopmentAxis: {
                ...
                futureExpectations: parsed.temporalDevelopmentAxis?.futureExpectations || "",
                heroArcConnection: parsed.temporalDevelopmentAxis?.heroArcConnection || "",
...
`;
        ...
        return (0, text_utils_1.toText)(result.content) || "فشل توليد العرض المختصر";
    }
        ...
        for (const [genre, contributions] of Object.entries(parsed)) {
            if (typeof contributions === "object" && contributions !== null) {
                result[genre] = {
                    ...
                    visualCompositionContribution: contributions.visualCompositionContribution || "",
    ...
    validateDynamicTone(parsed) {
        ...
        for (const stage of stages) {
            if (parsed[stage] && typeof parsed[stage] === "object") {
                result[stage] = {
                    visualAtmosphereDescribed: parsed[stage].visualAtmosphereDescribed || "",
                    writtenPacing: parsed[stage].writtenPacing || "",
                    dialogueStructure: parsed[stage].dialogueStructure || "",
                    soundIndicationsDescribed: parsed[stage].soundIndicationsDescribed || "",
...
`;
    ...
    calculateConfidenceScore(storyStatements, threeDMap, themeAnalysis) {
        ...
        const hasValidTemporal = threeDMap.temporalDevelopmentAxis.pastInfluence.length > 0;
