...
class Station7Finalization extends base_station_1.BaseStation {
    ...
    async process(input) {
        ...
        logger_1.default.info("[S7] Starting comprehensive final report generation...");
        try {
            ...
            const scoreMatrix = this.calculateScoreMatrix(station1, station2, station3, station4, station5, station6);
            const [executiveSummary, overallAssessment, swotAnalysis, audienceResonance, rewritingSuggestions, finalConfidence,] = await Promise.all([
                this.generateExecutiveSummary(station1, station2, station3, station4, station5, station6, scoreMatrix),
                this.generateOverallAssessment(scoreMatrix, station1, station2, station3, station4, station5, station6),
                ...
                this.calculateFinalConfidence(station1, station2, station3, station4, station5, station6),
            ]);
            const finalReport = {
                executiveSummary,
                overallAssessment,
            ...
            const agentsUsed = this.extractAgentsUsed(station1, station2, station3, station4, station5, station6);
            const tokensUsed = this.calculateTotalTokens(station1, station2, station3, station4, station5, station6);
            const output = {
                finalReport,
                scoreMatrix,
                finalConfidence,
            ...
            logger_1.default.info("[S7] Final report generation completed successfully");
            return output;
        }
        catch (error) {
    ...
    calculateScoreMatrix(s1, s2, s3, s4, s5, s6) {
        ...
        const overall = foundation * weights.foundation +
    ...
    calculateStationScore(station) {
        if (!station)
            return 0;
        const scores = [];
        if (typeof station.confidence === "number")
            scores.push(station.confidence);
    ...
    calculateStation5Score(s5) {
        ...
        if (s5.stylisticAnalysisResults?.overallToneAssessment?.toneConsistency) {
            scores.push(s5.stylisticAnalysisResults.overallToneAssessment.toneConsistency * 10);
        }
        return scores.length > 0
            ? scores.reduce((a, b) => a + b) / scores.length
            : 50;
...
`;
        const response = await this.geminiService.generate({
            ...
            temperature: 0.4,
            maxTokens: 2048,
            systemInstruction: "أنت محلل استراتيجي متخصص في تحليل SWOT للأعمال الدرامية. قدم ردك بصيغة نصية منظمة فقط.",
        });
        try {
...
`;
        const response = await this.geminiService.generate({
            ...
            temperature: 0.5,
            maxTokens: 1024,
            systemInstruction: "أنت محلل جمهور متخصص في توقع استجابات الجمهور للأعمال الدرامية. قدم تقييماً نصياً منظماً فقط.",
        });
        try {
            const text = this.extractText(response.content);
...
`;
        const response = await this.geminiService.generate({
            ...
            temperature: 0.4,
            maxTokens: 4096,
            systemInstruction: "أنت مستشار كتابة إبداعية متخصص. قدم اقتراحات محددة وقابلة للتنفيذ بصيغة نصية منظمة.",
        });
        try {
            const text = this.extractText(response.content);
    ...
    async calculateFinalConfidence(s1, s2, s3, s4, s5, s6) {
        ...
        [s1, s2, s3, s4, s5, s6].forEach((station, index) => {
            if (station?.uncertaintyReport?.epistemicUncertainty) {
                epistemicUncertainties.push(...station.uncertaintyReport.epistemicUncertainty);
            }
            if (station?.uncertaintyReport?.aleatoricUncertainty) {
                aleatoricUncertainties.push(...station.uncertaintyReport.aleatoricUncertainty);
            }
    ...
    async saveReports(output) {
        try {
            ...
            await (0, saveText_1.saveText)(`${this.outputDir}/final-report.txt`, humanReadableReport);
            const markdownReport = this.generateMarkdownReport(output);
            await (0, saveText_1.saveText)(`${this.outputDir}/final-report.md`, markdownReport);
    ...
    parseAudienceResonance(text) {
        ...
        for (const line of lines) {
            ...
            if (trimmed.includes("التأثير العاطفي")) {
                ...
                if (match)
                    result.emotionalImpact = parseInt(match[1]);
            }
            else if (trimmed.includes("التفاعل الفكري")) {
                const match = trimmed.match(/(\d+)/);
                if (match)
                    result.intellectualEngagement = parseInt(match[1]);
            }
            else if (trimmed.includes("القابلية للارتباط")) {
                const match = trimmed.match(/(\d+)/);
                if (match)
                    result.relatability = parseInt(match[1]);
            }
            else if (trimmed.includes("قابلية التذكر")) {
                const match = trimmed.match(/(\d+)/);
                if (match)
                    result.memorability = parseInt(match[1]);
            }
            else if (trimmed.includes("الإمكانات الفيروسية")) {
                const match = trimmed.match(/(\d+)/);
                if (match)
                    result.viralPotential = parseInt(match[1]);
            }
