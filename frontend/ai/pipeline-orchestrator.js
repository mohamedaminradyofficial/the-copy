...
exports.runPipelineWithInterfaces = runPipelineWithInterfaces;
exports.getStationOutput = getStationOutput;
exports.exportStationText = exportStationText;
exports.exportFinalReport = exportFinalReport;
const gemini_core_1 = require("./gemini-core");
// =====================================================
// Station 1: Basic Text Analysis
// =====================================================
async function station1_basic(text) {
...
النص:
${(0, gemini_core_1.safeSub)(text, 0, 30000)}

قدم تحليلاً نصياً مفصلاً بدون JSON.`;
    return await (0, gemini_core_1.callGeminiText)({
...
// =====================================================
async function station2_conceptual(context) {
...
${(0, gemini_core_1.safeSub)(context, 0, 35000)}
...
// =====================================================
async function station3_network(context) {
...
${(0, gemini_core_1.safeSub)(context, 0, 40000)}
...
// =====================================================
async function station4_efficiency(context) {
...
السياق الكامل:
${(0, gemini_core_1.safeSub)(context, 0, 45000)}
...
// =====================================================
async function station5_dynamic(context) {
...
// =====================================================
async function station6_diagnostics(context) {
...
${(0, gemini_core_1.safeSub)(context, 0, 47000)}
...
// =====================================================
async function station7_finalize(context) {
...
${(0, gemini_core_1.safeSub)(context, 0, 48000)}
...
async function runPipeline(text) {
    ...
    try {
        ...
        c.s3 = await station3_network(`${c.s2}\n\n[من مخرجات المحطة 1: ${(0, gemini_core_1.safeSub)(c.s1, 0, 2000)}]`);
        ...
        c.s6 = await station6_diagnostics(`${c.s1}\n${c.s2}\n${c.s3}\n${c.s4}\n${c.s5}`);
        ...
        c.s7 = await station7_finalize(`${c.s1}\n${c.s2}\n${c.s3}\n${c.s4}\n${c.s5}\n${c.s6}`);
...
/**
 * Run pipeline with new interface (returns structured StationOutput[])
 * This is the new preferred method that uses the interfaces from lib/ai/interfaces/stations.ts
 */
async function runPipelineWithInterfaces(text) {
    const outputs = [];
    const startTime = Date.now();
    try {
        // Station 1
        const s1Text = await station1_basic(text);
        outputs.push({
            stationId: "station-1",
            stationName: "التحليل النصي الأساسي",
            textOutput: s1Text,
            success: true,
        });
        // Station 2
        const s2Text = await station2_conceptual(`${s1Text}\n\n===\n${text}`);
        outputs.push({
            stationId: "station-2",
        ...
        // Station 3
        const s3Text = await station3_network(`${s2Text}\n\n[من مخرجات المحطة 1: ${(0, gemini_core_1.safeSub)(s1Text, 0, 2000)}]`);
        outputs.push({
            stationId: "station-3",
            stationName: "بناء شبكة الصراع",
            textOutput: s3Text,
            success: true,
        });
        // Station 4
        const s4Text = await station4_efficiency(`${s1Text}\n${s2Text}\n${s3Text}`);
        outputs.push({
            stationId: "station-4",
            stationName: "مقاييس الكفاءة",
            textOutput: s4Text,
            success: true,
        });
        // Station 5
        const s5Text = await station5_dynamic(`${s2Text}\n${s3Text}\n${s4Text}`);
        outputs.push({
            stationId: "station-5",
        ...
        // Station 6
        const s6Text = await station6_diagnostics(`${s1Text}\n${s2Text}\n${s3Text}\n${s4Text}\n${s5Text}`);
        outputs.push({
            stationId: "station-6",
            stationName: "التشخيص والعلاج",
            textOutput: s6Text,
            success: true,
        });
        // Station 7
        const s7Text = await station7_finalize(`${s1Text}\n${s2Text}\n${s3Text}\n${s4Text}\n${s5Text}\n${s6Text}`);
        outputs.push({
            stationId: "station-7",
        ...
        return {
            success: false,
            fullReport: "فشل في إتمام التحليل: " + errorMsg,
            stationOutputs: outputs,
            processingTime,
            notes: errorMsg,
        };
 ...
 */
function getStationOutput(result, stationNum) {
    const key = `s${stationNum}`;
    const output = result.stations[key];
    return (0, gemini_core_1.toText)(output);
}
/**
 * Export single station to text file
 */
function exportStationText(result, stationNum) {
 ...
 */
function exportFinalReport(result) {
    const sections = [
        "===========================================",
