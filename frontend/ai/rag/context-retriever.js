...
exports.getContextRetriever = getContextRetriever;
class ContextRetriever {
     ...
     */
    async retrieveRelevantChunks(query, chunks, contextMap, options = {}) {
        ...
        for (const chunk of chunks) {
            const score = await this.calculateRelevanceScore(query, chunk, contextMap);
            relevanceScores.set(chunk.id, score);
        }
        // ترتيب الأجزاء حسب درجة الصلة
        const sortedChunks = chunks
            .filter((chunk) => (relevanceScores.get(chunk.id) || 0) >= minRelevanceScore)
     ...
     */
    async generateChunkSummary(content) {
        const prompt = `
...
`;
     ...
     * استخراج العلاقات بين الكيانات
     */
    async extractRelationships(chunks) {
     ...
     */
    async calculateRelevanceScore(query, chunk, contextMap) {
        ...
        const combinedScore = wordSimilarity * 0.5 + entityScore * 0.3 + themeScore * 0.2;
        return Math.min(combinedScore, 1);
    }
}
exports.ContextRetriever = ContextRetriever;
// Export singleton instance
let contextRetrieverInstance = null;
function getContextRetriever(geminiService) {
