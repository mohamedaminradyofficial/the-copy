...
class TextChunker {
    ...
    splitLargeChunk(text, baseStart, chunkIndex) {
        const subChunks = [];
    ...
    buildContextPrompt(contextMap, currentChunkId) {
        const currentChunk = contextMap.chunks.find((c) => c.id === currentChunkId);
        const chunkIndex = contextMap.chunks.findIndex((c) => c.id === currentChunkId);
        let contextPrompt = `
## سياق النص الكامل:
