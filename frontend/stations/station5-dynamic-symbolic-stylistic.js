...
exports.Station5DynamicSymbolicStylistic = void 0;
const base_station_1 = require("../core/pipeline/base-station");
const base_entities_1 = require("../core/models/base-entities");
...
class DynamicAnalysisEngine {
    ...
    async constructEventTimeline(network) {
        const events = [];
        ...
        for (const snapshot of network.snapshots) {
            events.push({
                ...
                involvedEntities: {},
                significance: 5,
                narrativePhase: this.inferNarrativePhase(snapshot.timestamp, network.snapshots),
            });
        }
        // Process conflicts
    ...
    inferNarrativePhase(timestamp, snapshots) {
    ...
    async analyzeNetworkEvolution(network, timeline) {
        ...
        const overallGrowthRate = complexityProgression.length > 1 && lastComplexity && firstComplexity
            ? (lastComplexity - firstComplexity) / complexityProgression.length
            : 0;
        const stabilityMetrics = this.calculateStabilityMetrics(complexityProgression, densityProgression);
        return {
            overallGrowthRate,
    ...
    async trackCharacterDevelopment(network, timeline) {
        const evolutionMap = new Map();
        for (const [charId, character] of Array.from(network.characters.entries())) {
            const developmentStages = [];
            const keyMoments = [];
            for (const snapshot of network.snapshots) {
                if (!snapshot.networkState.characters?.has(charId))
                    continue;
            ...
            const transformationScore = this.calculateTransformationScore(developmentStages);
    ...
    calculateTransformationScore(stages) {
        ...
        for (let i = 1; i < stages.length; i++) {
            ...
            if (!prev || !curr)
                continue;
            const conflictChange = Math.abs(curr.conflicts.length - prev.conflicts.length);
            const relationshipChange = Math.abs(curr.relationships.length - prev.relationships.length);
            totalChange += conflictChange + relationshipChange;
        }
        return Math.min(10, totalChange / stages.length);
    }
    async trackConflictProgression(network, timeline) {
        ...
        for (const [confId, conflict] of Array.from(network.conflicts.entries())) {
            ...
            for (const snapshot of network.snapshots) {
                if (!snapshot.networkState.conflicts?.has(confId))
                    continue;
                ...
                if (previousPhase !== null && confState.phase !== previousPhase) {
                    const catalyst = timeline.find((event) => event.timestamp.getTime() === snapshot.timestamp.getTime() &&
                        event.involvedEntities.conflicts?.includes(confId))?.description || "Unknown catalyst";
            ...
            const resolutionProbability = this.calculateResolutionProbability(conflict, phaseTransitions);
            const stagnationRisk = this.calculateStagnationRisk(intensityProgression, phaseTransitions);
    ...
    calculateResolutionProbability(conflict, transitions) {
        let probability = 0.5;
        if (conflict.phase === base_entities_1.ConflictPhase.RESOLUTION) {
            probability = 0.95;
        }
        else if (conflict.phase === base_entities_1.ConflictPhase.DEESCALATING) {
            probability = 0.75;
        }
        else if (conflict.phase === base_entities_1.ConflictPhase.AFTERMATH) {
            probability = 1.0;
        }
        else if (conflict.phase === base_entities_1.ConflictPhase.CLIMAX) {
            probability = 0.6;
        }
        else if (conflict.phase === base_entities_1.ConflictPhase.LATENT) {
            probability = 0.2;
        }
    ...
    calculateStagnationRisk(intensityProgression, transitions) {
        if (intensityProgression.length < 3)
            return 0.5;
    ...
    `;
        try {
            const result = await this.geminiService.generate({
                prompt,
                context: (0, gemini_core_1.safeSub)(fullText, 0, 30000),
            ...
            return {
                ...
                centralThemesHintedBySymbols: analysis.central_themes_hinted_by_symbols || [],
...
class StylisticAnalysisEngine {
    ...
    `;
        try {
            ...
            return {
                toneAssessment: {
                    primaryTone: analysis.overall_tone_assessment?.primary_tone || "Unknown",
                    secondaryTones: analysis.overall_tone_assessment?.secondary_tones || [],
                    toneConsistency: analysis.overall_tone_assessment?.tone_consistency || 5,
                    explanation: analysis.overall_tone_assessment?.explanation || "Analysis failed",
                },
                languageComplexity: {
                    level: analysis.language_complexity?.level || "moderate",
                    readabilityScore: analysis.language_complexity?.readability_score || 5,
                    vocabularyRichness: analysis.language_complexity?.vocabulary_richness || 5,
                },
                pacingAnalysis: {
                    overallPacing: analysis.pacing_impression?.overall_pacing || "balanced",
                    pacingVariation: analysis.pacing_impression?.pacing_variation || 5,
                    sceneLengthDistribution: analysis.pacing_impression?.scene_length_distribution || [],
                },
                descriptiveRichness: {
                    visualDetailLevel: analysis.descriptive_richness?.visual_detail_level || 5,
                    sensoryEngagement: analysis.descriptive_richness?.sensory_engagement || 5,
                    atmosphericQuality: analysis.descriptive_richness?.atmospheric_quality || 5,
                },
            };
        }
        catch (error) {
...
class TensionAnalysisEngine {
    ...
    async analyzeTension(fullText, network) {
        const prompt = `
    ...
    `;
        try {
            ...
            const peaks = (analysis.peaks || []).map((peak) => ({
                ...peak,
                timestamp: new Date(peak.timestamp * 100000000), // Convert to a date
            }));
            const valleys = (analysis.valleys || []).map((valley) => ({
                ...valley,
                timestamp: new Date(valley.timestamp * 100000000), // Convert to a date
            }));
            const addTension = (analysis.recommendations?.add_tension || []).map((loc) => ({
                ...loc,
                timestamp: new Date(loc.timestamp * 100000000), // Convert to a date
            }));
            const reduceTension = (analysis.recommendations?.reduce_tension || []).map((loc) => ({
                ...loc,
                timestamp: new Date(loc.timestamp * 100000000), // Convert to a date
            }));
            return {
                ...
                recommendations: {
                    addTension,
                    reduceTension,
                    redistributeTension: analysis.recommendations?.redistribute_tension || [],
                },
            };
        }
        catch (error) {
...
class AdvancedDialogueAnalysisEngine {
    ...
    async analyzeDialogue(fullText, characters) {
        const prompt = `
    ...
    `;
        try {
            ...
            const emotionalBeats = (analysis.emotional_beats || []).map((beat) => ({
                ...beat,
                timestamp: new Date(beat.timestamp * 100000000), // Convert to a date
            }));
            // Convert character voice consistency to a Map
            const characterVoiceConsistency = new Map();
            if (analysis.advanced_metrics?.character_voice_consistency) {
                for (const [char, consistency] of Object.entries(analysis.advanced_metrics.character_voice_consistency)) {
                    characterVoiceConsistency.set(char, consistency);
                }
            }
            return {
...
class VisualCinematicAnalysisEngine {
    ...
    async analyzeVisualCinematic(fullText) {
        const prompt = `
    ...
    `;
        try {
            ...
            const analysis = JSON.parse(result.content || "{}");
            // Convert timestamps to actual dates
            const keyVisualMoments = (analysis.key_visual_moments || []).map((moment) => ({
                ...moment,
 ...
 */
class Station5DynamicSymbolicStylistic extends base_station_1.BaseStation {
    constructor(config, geminiService) {
        ...
        this.debateSystem = new constitutional_1.MultiAgentDebateSystem(geminiService);
    }
    async process(input) {
        ...
        const agentsUsed = [];
        // Dynamic Analysis
        const eventTimeline = await this.dynamicEngine.constructEventTimeline(input.conflictNetwork);
        agentsUsed.push("DynamicAnalysisEngine");
        const networkEvolution = await this.dynamicEngine.analyzeNetworkEvolution(input.conflictNetwork, eventTimeline);
        const characterDevelopment = await this.dynamicEngine.trackCharacterDevelopment(input.conflictNetwork, eventTimeline);
        const conflictProgression = await this.dynamicEngine.trackConflictProgression(input.conflictNetwork, eventTimeline);
        ...
        // Symbolic Analysis
        const symbolicAnalysis = await this.symbolicEngine.analyzeSymbols(input.fullText);
        agentsUsed.push("SymbolicAnalysisEngine");
        // Stylistic Analysis
        const stylisticAnalysis = await this.stylisticEngine.analyzeStyle(input.fullText);
        agentsUsed.push("StylisticAnalysisEngine");
        // Tension Analysis
        const tensionAnalysis = await this.tensionEngine.analyzeTension(input.fullText, input.conflictNetwork);
        agentsUsed.push("TensionAnalysisEngine");
        // Advanced Dialogue Analysis
        const advancedDialogueAnalysis = await this.dialogueEngine.analyzeDialogue(input.fullText, input.conflictNetwork.characters);
        agentsUsed.push("AdvancedDialogueAnalysisEngine");
        // Visual & Cinematic Analysis
        const visualCinematicAnalysis = await this.visualEngine.analyzeVisualCinematic(input.fullText);
        ...
        if (options.enableUncertaintyQuantification) {
            ...
            const uncertaintyMetrics = await constitutional_1.uncertaintyQuantificationEngine.quantify(analysisText, {
        ...
        if (options.enableMultiAgentDebate) {
            ...
            debateResults = await this.debateSystem.conductDebate(input.fullText, analysisText, {
                analysisType: "Dynamic, Symbolic, and Stylistic Analysis",
            });
        ...
        if (options.enableConstitutionalAI) {
            ...
            const constitutionalCheck = await (0, constitutional_1.checkConstitutionalCompliance)(analysisText, input.fullText, this.geminiService);
...
exports.Station5DynamicSymbolicStylistic = Station5DynamicSymbolicStylistic;
