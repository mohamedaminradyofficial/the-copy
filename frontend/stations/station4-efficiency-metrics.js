...
class Station4EfficiencyMetrics extends base_station_1.BaseStation {
    ...
    async process(input) {
        ...
        const options = {
            ...
            enableUncertaintyQuantification: input.options?.enableUncertaintyQuantification ?? true,
            temperature: input.options?.temperature ?? 0.3,
        };
        try {
            // 1. Calculate efficiency metrics
            const efficiencyMetrics = await this.calculateEfficiencyMetrics(input.station3Output.conflictNetwork);
            agentsUsed.push("EfficiencyMetrics");
            // 2. Assess literary quality
            const literaryQuality = await this.literaryQualityAnalyzer.assess(input.originalText, input.station3Output);
            agentsUsed.push("LiteraryQualityAnalyzer");
            // 3. Evaluate producibility
            const producibilityAnalysis = await this.producibilityAnalyzer.evaluate(input.originalText, input.station3Output.conflictNetwork);
            agentsUsed.push("ProducibilityAnalyzer");
            // 4. Analyze rhythm
            const rhythmAnalysis = await this.rhythmMappingAgent.analyze(input.originalText);
            ...
            const qualityAssessment = {
                ...
                commercial: this.calculateCommercialPotential(literaryQuality, producibilityAnalysis),
            ...
            const recommendations = await this.recommendationsGenerator.generate(efficiencyMetrics, qualityAssessment, producibilityAnalysis, rhythmAnalysis);
            agentsUsed.push("RecommendationsGenerator");
            // 7. Apply Constitutional AI if enabled
            let finalRecommendations = recommendations;
            if (options.enableConstitutionalAI) {
                const constitutionalCheck = await this.checkConstitutionalCompliance(JSON.stringify(recommendations), input.originalText);
                if (!constitutionalCheck.compliant) {
                    finalRecommendations = JSON.parse(constitutionalCheck.correctedAnalysis);
                }
            ...
            if (options.enableUncertaintyQuantification) {
                uncertaintyReport = await this.quantifyUncertainty(JSON.stringify({
                    efficiencyMetrics,
                    qualityAssessment,
    ...
    async calculateEfficiencyMetrics(conflictNetwork) {
        ...
        const characterEfficiency = Math.min(100, Math.max(0, 100 - (charactersCount - 7) * 5));
        const relationshipEfficiency = Math.min(100, Math.max(0, 100 - (relationshipsCount - charactersCount * 1.5) * 3));
        const conflictEfficiency = Math.min(100, Math.max(0, 100 - (conflictsCount - charactersCount * 0.8) * 4));
        ...
        const overallEfficiencyScore = characterEfficiency * 0.3 +
            relationshipEfficiency * 0.3 +
            conflictEfficiency * 0.4;
        // Determine overall rating
        let overallRating;
        if (overallEfficiencyScore >= 80)
            overallRating = "Excellent";
        else if (overallEfficiencyScore >= 60)
            overallRating = "Good";
        else if (overallEfficiencyScore >= 40)
            overallRating = "Fair";
        else if (overallEfficiencyScore >= 20)
            overallRating = "Poor";
        else
            overallRating = "Critical";
        // Calculate conflict cohesion (simplified)
        const conflictCohesion = Math.min(1, Math.max(0, conflictsCount / charactersCount));
        // Calculate dramatic balance
        const dramaticBalance = {
            balanceScore: Math.min(1, Math.max(0, 1 - Math.abs(1 - conflictsCount / charactersCount))),
            characterInvolvementGini: this.calculateGiniCoefficient(conflictNetwork),
        };
        // Calculate narrative density
        const narrativeDensity = (conflictsCount + relationshipsCount) / charactersCount;
        // Calculate redundancy metrics
        const redundancyMetrics = {
            characterRedundancy: Math.max(0, (charactersCount - 7) / charactersCount),
            relationshipRedundancy: Math.max(0, (relationshipsCount - charactersCount * 1.5) / relationshipsCount),
            conflictRedundancy: Math.max(0, (conflictsCount - charactersCount * 0.8) / conflictsCount),
        };
        return {
    ...
    calculateCommercialPotential(literaryQuality, producibility) {
        ...
        let budgetScore = 50;
        if (producibility.budgetEstimate === "low")
            budgetScore = 40;
        else if (producibility.budgetEstimate === "medium")
            budgetScore = 70;
        else if (producibility.budgetEstimate === "high")
            budgetScore = 60;
        else if (producibility.budgetEstimate === "very_high")
            budgetScore = 30;
        // Cast size factor (5-10 is optimal)
        let castScore = 50;
        if (producibility.castSize <= 5)
            castScore = 40;
        else if (producibility.castSize <= 10)
            castScore = 70;
        else if (producibility.castSize <= 15)
            castScore = 60;
        else
            castScore = 30;
        return Math.round(literaryScore * 0.4 +
            technicalScore * 0.2 +
            budgetScore * 0.2 +
            castScore * 0.2);
    }
    async checkConstitutionalCompliance(analysis, originalText) {
        try {
            return await (0, constitutional_1.constitutionalCheck)(analysis, originalText, this.geminiService);
        }
        catch (error) {
    ...
    async quantifyUncertainty(analysis, originalText) {
        try {
            const uncertainty = await (0, constitutional_1.quantifyUncertainty)(analysis, {
                originalText,
    ...
    extractRequiredData(input) {
        return {
            ...
            relationshipsCount: input.station3Output.conflictNetwork.relationships.size,
