...
exports.Station3NetworkBuilder = exports.NetworkDiagnostics = exports.ConflictNetworkImpl = exports.ConflictPhase = exports.ConflictScope = exports.ConflictSubject = exports.RelationshipDirection = exports.RelationshipNature = exports.RelationshipType = void 0;
...
اكتب تحليلاً مفصلاً للعلاقات الرئيسية بين الشخصيات.
    `;
        ...
        const inferredRelationships = this.parseRelationshipsFromText(analysisText, characters);
    ...
    parseRelationshipsFromText(text, characters) {
        ...
        for (const pattern of relationshipPatterns) {
            ...
            while ((match = pattern.exec(text)) !== null) {
                ...
                if (!char1Name || !char2Name || !relationshipDesc)
                    continue;
    ...
    inferRelationshipNature(description) {
        const desc = description.toLowerCase();
        if (desc.includes("إيجابي") ||
            desc.includes("داعم") ||
            desc.includes("جيد")) {
            return RelationshipNature.POSITIVE;
        }
        else if (desc.includes("سلبي") ||
            desc.includes("معادي") ||
            desc.includes("سيء")) {
            return RelationshipNature.NEGATIVE;
        }
        else if (desc.includes("متغير") ||
            desc.includes("متبدل") ||
    ...
    buildContextSummary(context, station2Summary) {
        const characterProfiles = Array.from(context.characterProfiles?.entries() ?? []).map(([name, profile]) => ({
            name,
...
اكتب تحليلاً مفصلاً للصراعات الرئيسية في النص.
    ...
    inferConflictSubject(description) {
        ...
        else if (desc.includes("سلطة") ||
            desc.includes("سيطرة") ||
            desc.includes("حكم")) {
            return ConflictSubject.POWER;
        }
        else if (desc.includes("معتقد") ||
            desc.includes("رأي") ||
            desc.includes("فكر")) {
        ...
        else if (desc.includes("معلومات") ||
            desc.includes("سر") ||
            desc.includes("حقيقة")) {
            return ConflictSubject.INFORMATION;
        }
        else if (desc.includes("مكان") ||
            desc.includes("أرض") ||
            desc.includes("منزل")) {
            return ConflictSubject.TERRITORY;
        }
        else if (desc.includes("شرف") ||
            desc.includes("كرامة") ||
            desc.includes("سمعة")) {
    ...
    inferConflictScope(description) {
        const desc = description.toLowerCase();
        if (desc.includes("عالم") ||
            desc.includes("مجتمع") ||
            desc.includes("دولة")) {
            return ConflictScope.SOCIETAL;
        }
        else if (desc.includes("مجموعة") ||
            desc.includes("فريق") ||
            desc.includes("عائلة")) {
...
class NetworkAnalyzer {
    ...
    async analyzeNetwork(network, context) {
        // حساب كثافة الشبكة
        const maxPossibleConnections = (network.characters.size * (network.characters.size - 1)) / 2;
        ...
        const density = maxPossibleConnections > 0
            ...
            : 0;
        // حساب تعقيد الشبكة
        const avgConnectionsPerCharacter = network.characters.size > 0
        ...
        const strengthValues = Array.from(network.relationships.values()).map((rel) => rel.strength);
    ...
    calculateConflictDistribution(network) {
        ...
        for (const character of network.characters.values()) {
            const conflictCount = Array.from(network.conflicts.values()).filter((conflict) => conflict.involvedCharacters.includes(character.id)).length;
    ...
    async analyzeConflicts(network, context) {
        ...
        const mainConflict = conflicts.reduce((strongest, current) => current.strength > strongest.strength ? current : strongest, conflicts[0] || this.createDefaultConflict());
        // تحديد الصراعات الثانوية
        const subConflicts = conflicts.filter((conflict) => conflict.id !== mainConflict.id);
    ...
    async generateCharacterArcs(network, context) {
        ...
        for (const character of network.characters.values()) {
            const conflicts = Array.from(network.conflicts.values()).filter((conflict) => conflict.involvedCharacters.includes(character.id));
            const relationships = Array.from(network.relationships.values()).filter((rel) => rel.source === character.id || rel.target === character.id);
            const arc = {
    ...
    inferArcType(character, conflicts, relationships) {
        ...
        const conflictStrength = conflicts.reduce((sum, conflict) => sum + conflict.strength, 0);
    ...
    describeTransformation(character, conflicts, relationships) {
        ...
        const positiveRelationships = relationships.filter((rel) => rel.nature === RelationshipNature.POSITIVE).length;
        const negativeRelationships = relationships.filter((rel) => rel.nature === RelationshipNature.NEGATIVE).length;
...
class Station3NetworkBuilder extends base_station_1.BaseStation {
    constructor(geminiService) {
        ...
        this.networkDiagnostics = new NetworkDiagnostics(new ConflictNetworkImpl("temp", "temp"));
    }
    async execute(input, options) {
        if (!("station1Output" in input &&
        ...
        // إنشاء الشبكة
        const network = new ConflictNetworkImpl(`network_${Date.now()}`, `${(0, text_utils_1.safeSub)(station3Input.station2Output.storyStatement, 0, 50)}...`);
        ...
        // استنتاج العلاقات
        const relationships = await this.relationshipEngine.inferRelationships(characters, context, station3Input.station2Output);
        relationships.forEach((rel) => network.addRelationship(rel));
        // استنتاج الصراعات
        const conflicts = await this.conflictEngine.inferConflicts(characters, relationships, context, station3Input.station2Output);
        conflicts.forEach((conflict) => network.addConflict(conflict));
        // إنشاء لقطة أولية
        network.createSnapshot("Initial network state after AI inference");
        // تحليل الشبكة
        const networkAnalysis = await this.networkAnalyzer.analyzeNetwork(network, context);
        // تحليل الصراعات
        const conflictAnalysis = await this.networkAnalyzer.analyzeConflicts(network, context);
        // توليد أقواس الشخصيات
        const characterArcs = await this.networkAnalyzer.generateCharacterArcs(network, context);
        // تحديد نقاط التحول
        const pivotPoints = await this.networkAnalyzer.identifyPivotPoints(network, context);
        // تشخيص الشبكة
