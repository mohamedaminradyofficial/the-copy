...
exports.Station6Diagnostics = void 0;
const multi_agent_debate_1 = require("../constitutional/multi-agent-debate");
// =====================================================
// MAIN CLASS
// =====================================================
class Station6Diagnostics {
    constructor(geminiService) {
        this.geminiService = geminiService;
        this.debateSystem = new multi_agent_debate_1.MultiAgentDebateSystem(geminiService);
        this.uncertaintyEngine = new uncertainty_quantification_1.UncertaintyQuantificationEngine(geminiService);
    }
    /**
     * Execute comprehensive diagnostics and treatment analysis
     */
    async execute(text, previousStationsOutput) {
        console.log("[Station 6] Starting comprehensive diagnostics");
        const startTime = Date.now();
        try {
            // Generate comprehensive diagnostics report
            const diagnosticsReport = await this.generateComprehensiveDiagnostics(text, previousStationsOutput);
            // Conduct multi-agent debate for critical validation
            const debateResults = await this.conductValidationDebate(text, previousStationsOutput, diagnosticsReport);
            // Generate detailed treatment plan
            const treatmentPlan = await this.generateDetailedTreatmentPlan(diagnosticsReport, debateResults, previousStationsOutput);
            // Predict plot trajectory with alternatives
            const plotPredictions = await this.predictPlotTrajectoryWithAlternatives(text, previousStationsOutput, diagnosticsReport);
            ...
            console.log(`[Station 6] Analysis completed in ${metadata.executionTime}ms`);
            return {
                diagnosticsReport,
                debateResults,
                treatmentPlan,
            ...
            throw new Error(`Station 6 execution failed: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    // =====================================================
     ...
     */
    async generateComprehensiveDiagnostics(text, previousStationsOutput) {
        const analysisSummary = this.createStructuredAnalysisSummary(previousStationsOutput);
        const prompt = `
...
`;
     ...
     */
    validateAndEnrichDiagnostics(data) {
        return {
            overallHealthScore: Math.min(Math.max(data.overallHealthScore || 50, 0), 100),
            healthBreakdown: {
                characterDevelopment: Math.min(Math.max(data.healthBreakdown?.characterDevelopment || 50, 0), 100),
                plotCoherence: Math.min(Math.max(data.healthBreakdown?.plotCoherence || 50, 0), 100),
                structuralIntegrity: Math.min(Math.max(data.healthBreakdown?.structuralIntegrity || 50, 0), 100),
                dialogueQuality: Math.min(Math.max(data.healthBreakdown?.dialogueQuality || 50, 0), 100),
                thematicDepth: Math.min(Math.max(data.healthBreakdown?.thematicDepth || 50, 0), 100),
            },
     ...
     */
    generateFallbackDiagnostics(previousStationsOutput) {
        const efficiencyScore = previousStationsOutput.station4?.efficiencyMetrics
        ...
        return {
            ...
            riskAreas: [],
            opportunities: [],
            summary: "فشل التحليل التشخيصي التفصيلي. تم استخدام بيانات أساسية من المحطات السابقة.",
        };
    }
    // =====================================================
    // MULTI-AGENT DEBATE
     ...
     */
    async conductValidationDebate(text, previousStationsOutput, diagnosticsReport) {
        const debateContext = `
**التقرير التشخيصي:**
...
`;
        try {
            return await this.debateSystem.conductDebate(text.substring(0, 3000), debateContext, {
     ...
     */
    async generateDetailedTreatmentPlan(diagnosticsReport, debateResults, previousStationsOutput) {
        const prompt = `
...
`;
     ...
     */
    validateAndEnrichTreatmentPlan(data) {
        return {
            prioritizedRecommendations: (data.prioritizedRecommendations || []).slice(0, 20),
            ...
            estimatedImprovementScore: Math.min(Math.max(data.estimatedImprovementScore || 50, 0), 100),
     ...
     */
    generateFallbackTreatmentPlan(diagnosticsReport) {
        ...
        return {
            ...
            estimatedImprovementScore: Math.min(diagnosticsReport.overallHealthScore + 30, 100),
            implementationComplexity: diagnosticsReport.criticalIssues.length > 5 ? "high" : "medium",
     ...
     */
    async predictPlotTrajectoryWithAlternatives(text, previousStationsOutput, diagnosticsReport) {
        const contextSummary = this.createStructuredAnalysisSummary(previousStationsOutput);
        const prompt = `
...
`;
     ...
     */
    validateAndEnrichPlotPredictions(data) {
        return {
            currentTrajectory: (data.currentTrajectory || []).slice(0, 10),
            trajectoryConfidence: Math.min(Math.max(data.trajectoryConfidence || 0.5, 0), 1),
            ...
            predictabilityScore: Math.min(Math.max(data.predictabilityScore || 5, 0), 10),
        };
    }
    /**
     * Generate fallback plot predictions
     */
    generateFallbackPlotPredictions(previousStationsOutput) {
...
`;
     ...
     */
    createStructuredAnalysisSummary(previousStationsOutput) {
        return `
...
exports.Station6Diagnostics = Station6Diagnostics;
