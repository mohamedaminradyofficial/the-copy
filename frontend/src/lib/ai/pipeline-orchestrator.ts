...
import type { StationOutput, OrchestratorResult } from "./interfaces/stations";
...
export async function runPipelineWithInterfaces(
  ...
  try {
    ...
    // Station 3
    const s3Text = await station3_network(
      `${s2Text}\n\n[من مخرجات المحطة 1: ${safeSub(s1Text, 0, 2000)}]`
    );
    outputs.push({
      stationId: "station-3",
    ...
    // Station 6
    const s6Text = await station6_diagnostics(
      `${s1Text}\n${s2Text}\n${s3Text}\n${s4Text}\n${s5Text}`
    );
    outputs.push({
      stationId: "station-6",
    ...
    // Station 7
    const s7Text = await station7_finalize(
      `${s1Text}\n${s2Text}\n${s3Text}\n${s4Text}\n${s5Text}\n${s6Text}`
    );
    outputs.push({
      stationId: "station-7",
