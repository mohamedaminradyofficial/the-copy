...
class GeminiService {
  ...
  async generate(request) {
    ...
    if (this.config.enableCaching) {
      ...
      if (cached) {
        logger_1.default.info("[GeminiService] Returning cached response", {
          cacheKey,
        });
        return cached;
      }
  ...
  async performRequest(request, attemptNumber) {
    ...
    logger_1.default.info(
      `[GeminiService] Generating content with model ${modelName}`,
      {
    ...
    if (validator) {
      ...
      if (allowPartial && onPartialFallback) {
        ...
        if (partial !== undefined) {
          logger_1.default.info(
            "[GeminiService] Using partial fallback for response"
          );
          return partial;
        }
      }
  ...
  async enforceRateLimit() {
    ...
    if (
      ...
      logger_1.default.warn(
        `[GeminiService] Rate limit reached. Waiting ${waitTime}ms`,
        {
