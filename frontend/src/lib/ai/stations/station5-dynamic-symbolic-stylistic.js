  ...
  async analyzeTension(fullText, network) {
    ...
    try {
      ...
      const peaks = (analysis.peaks || []).map((peak) => ({
        ...peak,
        timestamp: new Date(peak.timestamp * 100000000),
      }));
      const valleys = (analysis.valleys || []).map((valley) => ({
        ...valley,
        timestamp: new Date(valley.timestamp * 100000000),
      }));
      const addTension = (analysis.recommendations?.add_tension || []).map(
        (loc) => ({
          ...loc,
          timestamp: new Date(loc.timestamp * 100000000),
        })
      );
      const reduceTension = (
  ...
  async analyzeDialogue(fullText, characters) {
    ...
    try {
      ...
      const emotionalBeats = (analysis.emotional_beats || []).map((beat) => ({
        ...beat,
        timestamp: new Date(beat.timestamp * 100000000),
      }));
  ...
  async analyzeVisualCinematic(fullText) {
    ...
    try {
      ...
      const keyVisualMoments = (analysis.key_visual_moments || []).map(
        (moment) => ({
          ...moment,
          timestamp: new Date(moment.timestamp * 100000000),
        })
      );
      return {
...
class Station5DynamicSymbolicStylistic extends base_station_1.BaseStation {
  ...
  constructor(config, geminiService) {
    ...
    this.debateSystem = new constitutional_1.MultiAgentDebateSystem(
      geminiService
    );
  }
  async process(input) {
    ...
    if (options.enableUncertaintyQuantification) {
      ...
      const uncertaintyMetrics =
        await constitutional_1.uncertaintyQuantificationEngine.quantify(
          analysisText,
          {
    ...
    if (options.enableConstitutionalAI) {
      ...
      const constitutionalCheck = await (0,
      constitutional_1.checkConstitutionalCompliance)(
        analysisText,
        input.fullText,
        this.geminiService
      );
...
//# sourceMappingURL=station5-dynamic-symbolic-stylistic.js.map
