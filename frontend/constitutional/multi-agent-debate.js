...
exports.MultiAgentDebateSystem = void 0;
exports.getMultiAgentDebateSystem = getMultiAgentDebateSystem;
class MultiAgentDebateSystem {
     ...
     */
    async conductDebate(text, analysis, context, maxRounds = 3) {
        console.log(`[Multi-Agent Debate] Starting debate with max ${maxRounds} rounds`);
        // Define participants
        const participants = [
            {
                role: "prosecutor",
        ...
        for (let i = 0; i < maxRounds; i++) {
            ...
            const round = await this.conductDebateRound(i + 1, text, analysis, context, rounds // Previous rounds for context
            );
            rounds.push(round);
     ...
     */
    async conductDebateRound(roundNumber, text, analysis, context, previousRounds) {
        ...
        const prosecutorArg = await this.generateProsecutorArgument(text, analysis, debateContext);
        // Defender's response
        const defenderArg = await this.generateDefenderArgument(text, analysis, prosecutorArg.argument, debateContext);
        // Judge's comments
        const judgeComments = await this.generateJudgeComments(prosecutorArg, defenderArg);
        return {
            round: roundNumber,
     ...
     * توليد حجة المدعي
     */
    async generateProsecutorArgument(text, analysis, debateContext) {
        const prompt = `
...
`;
     ...
     * توليد حجة المدافع
     */
    async generateDefenderArgument(text, analysis, prosecutorArgument, debateContext) {
        const prompt = `
...
`;
     ...
     */
    async generateJudgeComments(prosecutorArg, defenderArg) {
        const prompt = `
...
`;
     ...
     * توليد الحكم النهائي
     */
    async generateVerdict(text, analysis, rounds) {
...
`;
    ...
    async checkConvergence(rounds) {
        ...
        const lastDiff = Math.abs(lastRound.prosecutorArgument.strength -
            lastRound.defenderArgument.strength);
        const prevDiff = Math.abs(prevRound.prosecutorArgument.strength -
     ...
     */
    async identifyControversialTopics(rounds) {
        const topics = [];
        for (const round of rounds) {
            // Find topics where prosecutor and defender strongly disagree
            const strengthDiff = Math.abs(round.prosecutorArgument.strength - round.defenderArgument.strength);
            if (strengthDiff > 5) {
                // Extract topic from arguments
                const prosecutorTopics = this.extractTopics(round.prosecutorArgument.argument);
                const defenderTopics = this.extractTopics(round.defenderArgument.argument);
                // Find common topics (they're arguing about the same thing)
                const commonTopics = prosecutorTopics.filter((t) => defenderTopics.some((dt) => dt.includes(t) || t.includes(dt)));
     ...
     * تحليل حجة من نص
     */
    parseDebateArgument(text, participant) {
        const argumentMatch = text.match(/الحجة:\s*([^\n]+(?:\n(?!الأدلة|قوة)[^\n]+)*)/);
...
exports.MultiAgentDebateSystem = MultiAgentDebateSystem;
// Export singleton instance
let debateSystemInstance = null;
function getMultiAgentDebateSystem(geminiService) {
