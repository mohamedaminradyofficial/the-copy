...
### 0. الإجراء الأولي الإلزامي (Pre-Fix Governance)

1. **قفل التقرير المرجعي:** تأكيد أن `SECURITY_SCAN_REPORT.md` مؤشّر كنسخة مرجعية "مقفلة" مع ختم زمني وآلية اعتماد التعديلات.
2. **إنشاء/تحديث لوحة الأمن:** ضمان وجود لوحة "Security Remediation Board" (GitHub Projects أو Jira) بالأعمدة التالية:
   - Critical Fixes
   - High
   - Medium
   - To Verify
   - Done
3. **تحويل findings إلى تذاكر:** استخدام سكربت `scripts/parse_create_issues.py` أو القالب اليدوي لإنشاء تذكرة مستقلة لكل Finding وربطها باللوحة.
4. **تفعيل بوابة الدمج الأمنية:** التحقق من أن قاعدة "لا Merge عند فشل Security CI" مفعّلة في حماية الفروع ومذكورة بوضوح في `SECURITY.md`.
